<script lang="ts">
	...
	function fillTournaments() {
		data.tournaments.items.forEach((tournament: TournamentsResponse, z: number) => {
			...
			while (currentDate.isBefore(endDate, 'date') || currentDate.isSame(endDate, 'date')) {
...
<div class="flex flex-col mt-20 w-full md:px-8 items-center h-screen justify-center">
	<div class="w-full md:w-3/4 xl:w-1/2 bg-surface-100-800-token p-8">
		...
		<div class="flex flex-row">
			<button class="btn variant-filled-primary mr-auto" on:click={prevMonth}>Prev</button>
			<button class="btn variant-filled-primary ml-auto" on:click={nextMonth}>Next</button>
		</div>
	</div>
</div>
