<script>
	...
	export let settings = { offset: 0, limit: 5, size: 0, amounts: [1, 2, 5, 10], modes: [''] };
	...
	export let mode = settings.modes[0];
...
<div class="paginator {classesBase}" data-testid="paginator">
	...
	<label class="paginator-label {classesLabel}">
		<select bind:value={mode} on:change={() => { onChangeLength() }} class="paginator-select {classesSelect}" {disabled} aria-label="Select Amount">
			{#each settings.modes as mode}<option value={mode}>{mode}</option>{/each}
		</select>
	</label>
	<!-- Details -->
